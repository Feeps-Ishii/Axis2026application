...
const SQL_PATH = join(homedir(), "axis-2026-app/team/teamI.sql");
const OUT = "public/team-apps/teamI/data/seed.json";

let sql = readFileSync(SQL_PATH, "utf8");
sql = sql.replace(/﻿/g, "").replace(/​/g, ""); // BOM/ゼロ幅スペース除去
sql = sql.replace(/--[^\n]*/g, "");            // SQL行コメント除去（VALUES間に挟まるため）
...
mkdirSync("public/team-apps/teamI/data", { recursive: true });
writeFileSync(OUT, JSON.stringify(seed, null, 2));

console.log("=== 件数 ===");
Object.entries(seed).forEach(([k, v]) => console.log(`${k}: ${v.length}`));
console.log("\n=== サンプル ===");
for (const [k, key] of [["book","books"],["staff","staff"],["stock","stocks"],["pop","pops"],["order","orders"],["comment","comments"],["category","categories"],["store","stores"]]) {
  console.log(`${k}[0]:`, JSON.stringify(seed[key][0]));
}
